...
if len(rs_vul_list) == 0:
    ...
    with open(vulreport, "a") as report:
        ...
        report.close()
    report_content.append(f"\tComplete Vulnerability Report for {target} named {vulreport} is available under the same directory KPToolBoxScanner resides.")
...
report_content.append(f"\tTotal Number of Vulnerability Checks        : {len(tool_names)}")
report_content.append(f"\tTotal Number of Vulnerability Checks Skipped: {rs_skipped_checks}")
report_content.append(f"\tTotal Number of Vulnerabilities Detected    : {len(rs_vul_list)}")
report_content.append(f"\tTotal Time Elapsed for the Scan             : {display_time(int(rs_total_elapsed))}")

report_content.append(f"\tFor Debugging Purposes, You can view the complete output generated by all the tools named {debuglog} under the same directory.")
